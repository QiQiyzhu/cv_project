...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run HAT-L over all mandatory datasets.")
    parser.add_argument(
        "--datasets",
    ...
    parser.add_argument(
        "--output-root",
        default=str(PROJECT_ROOT / "outputs" / "part2" / "hat_l_mandatory"),
        help="Output root for the mandatory HAT-L run.",
    )
    parser.add_argument(
        "--checkpoint",
        default=None,
        help="Optional HAT-L checkpoint path or URL. If omitted, the runtime resolves the default HAT-L checkpoint.",
    )
    ...
    parser.add_argument(
        "--wild-prepare-only",
        action="store_true",
        help="Only extract and resize wild_video inputs, without running HAT-L inference.",
    )
    parser.add_argument(
        "--skip-existing",
        action="store_true",
