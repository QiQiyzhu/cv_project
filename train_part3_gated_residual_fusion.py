...
from vsr_part2 import create_vsr_inferencer, run_vsr_dataset_inference, save_summary
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Real-ESRGAN over all mandatory datasets.")
    parser.add_argument(
        "--datasets",
    ...
    parser.add_argument(
        "--output-root",
        default=str(PROJECT_ROOT / "outputs" / "part2" / "realesrgan_mandatory"),
        help="Output root for the mandatory Real-ESRGAN run.",
    )
    parser.add_argument(
        "--checkpoint",
        default=None,
        help="Optional Real-ESRGAN checkpoint path or URL. If omitted, the official default checkpoint is downloaded automatically.",
    )
    ...
    parser.add_argument(
        "--wild-prepare-only",
        action="store_true",
        help="Only extract and resize wild_video inputs, without running Real-ESRGAN inference.",
    )
    parser.add_argument(
        "--skip-existing",
        action="store_true",
...
def run_wild_video_inference(
    ...
    inferencer = None
    if not prepare_only:
        inferencer = create_vsr_inferencer(
